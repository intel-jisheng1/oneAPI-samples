  ...
  try {
    ...
    using FFTToShim =
        sycl::ext::intel::pipe<class FFFTToShimPipe,
                               std::array<ac_complex<float>, kParallelism>, 0>;
    using FFTToProbe =
        sycl::ext::intel::pipe<class FFTToProbePipe,
                               std::array<ac_complex<float>, kParallelism>, 0>;
    ...
    int i = 0;
      ...
      auto fft_event = q.single_task<class FFTKernel>(
          FFT<kLogN, kLogParallelism, ShimToFFT, FFTToShim, float, FFTToProbe>{
              inverse, shim_fullfft_data});

      auto ShimKernelStoreProbe_event = q.single_task<class ShimKernelStoreProbeKernel>(
          ShimKernelStoreProbe<kLogN, kLogParallelism, FFTToProbe, float>{shim_fullfft_data_hoist});
      ...
      fft_event.wait();
      shim_event_2.wait();
      ShimKernelStoreProbe_event.wait();
      ...
      std::string filename3 = "shim_fullfft_data_" + std::to_string(i) + ".txt";
      ...
      // Writing the array elements to the file 
      for (int i = 0; i < (kN * (kN / kParallelism) + kN / kParallelism - 1) * kParallelism; ++i) { 
          outfile3<< host_shim_fullfft_data[i].r() << " "<< host_shim_fullfft_data[i].i() << " "; 
      } 
