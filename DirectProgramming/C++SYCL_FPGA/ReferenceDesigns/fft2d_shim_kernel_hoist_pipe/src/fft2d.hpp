...
// transform
template <int logn, size_t points, typename T, typename PipeOut>
...
struct ShimKernelLoad {
  ac_complex<T> *loadFrom;

  ShimKernelLoad(ac_complex<T> *loadFrom_) : loadFrom(loadFrom_) {}
...
struct ShimKernelStore {
  ac_complex<T> *storeTo;

  ShimKernelStore(ac_complex<T> *storeTo_) : storeTo(storeTo_) {}
...
template <int logn, size_t log_points, typename PipeIn, typename T>
struct ShimKernelStoreProbe {
  ac_complex<T> *storeTo;

  ShimKernelStoreProbe(ac_complex<T> *storeTo_) : storeTo(storeTo_) {}
...
template <int logn, size_t log_points, typename PipeIn, typename PipeOut,
          typename T, typename PipeOutProbe>
struct FFT {
  int inverse;
  ac_complex<T> *storeTo;
  ...
  void operator()() const {
    ...
    for (unsigned i = 0; i < kN * (kN / kPoints) + kN / kPoints - 1; i++) {
      ...
      data =
          FFTStep<logn, kPoints, float, PipeOutProbe>(data, i % (kN / kPoints), fft_delay_elements, inverse);
